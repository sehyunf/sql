...
FROM (
	...
	HAVING COUNT(SELL_PRODUCT_ID) >= (
		SELECT MAX(COUNT(SELL_PRODUCT_ID))  
		FROM TBL_PRODUCT_ORDER
		GROUP BY SELL_PRODUCT_ID
	)
) TPO
...
(
	...
	FROM TBL_SELL_PRODUCT  
	WHERE SELL_PRODUCT_BRAND LIKE '%예일%'
) - (
	SELECT COUNT(ID)
	FROM TBL_PRODUCT_ORDER TPO
...
ON TPO.BUY_USER_ID = TSP.ID
JOIN (
	SELECT *
	FROM TBL_BUY_USER
	WHERE BUY_USER_AGE = 20
) TBU
ON TPO.SELL_PRODUCT_ID = TBU.ID;
...
SELECT DISTINCT SELL_PRODUCT_BRAND 
FROM TBL_SELL_PRODUCT TSP 
JOIN (
	SELECT *
	FROM TBL_PRODUCT_ORDER
	WHERE BUY_USER_ID IN(
		SELECT ID
		FROM TBL_BUY_USER
		WHERE BUY_USER_AGE = 20
	)
